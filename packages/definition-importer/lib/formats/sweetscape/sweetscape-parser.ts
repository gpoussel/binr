...
export class SweetscapeParser extends CstParser {
  ...
  private primaryExpression = this.RULE("primaryExpression", () => {
    this.OR([
      ...
      {
        ALT: () => {
          this.CONSUME(tokens.Parentof);
          this.CONSUME4(tokens.ParenthesisOpen);
          this.SUBRULE4(this.expressionOrTypeName);
          this.CONSUME4(tokens.ParenthesisClose);
        },
      },
      {
        ALT: () => {
      ...
      {
        ALT: () => {
          this.CONSUME(tokens.FunctionExists);
          this.CONSUME5(tokens.ParenthesisOpen);
          this.CONSUME(tokens.Identifier);
          this.CONSUME5(tokens.ParenthesisClose);
        },
      },
