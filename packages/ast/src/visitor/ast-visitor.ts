...
export abstract class AstVisitor {
  ...
  public visitExpression(node: Expression): boolean {
    if (node instanceof ArrayIndexExpression) {
      return this.visitArrayIndexExpression(node);
    }
    if (node instanceof ArrayInitializationExpression) {
      return this.visitArrayInitializationExpression(node);
    }
    ...
    if (node instanceof FunctionCallExpression) {
      return this.visitFunctionCallExpression(node);
    }
    ...
    if (node instanceof SizeofExpression) {
      return this.visitSizeofExpression(node);
    }
    ...
    if (node instanceof ValueExpression) {
      return this.visitValueExpression(node);
    }
    throw new Error();
  }

  public endVisitExpression(node: Expression): void {
    if (node instanceof ArrayIndexExpression) {
      this.endVisitArrayIndexExpression(node);
    } else if (node instanceof ArrayInitializationExpression) {
      this.endVisitArrayInitializationExpression(node);
    } else if (node instanceof AssignmentExpression) {
      this.endVisitAssignmentExpression(node);
    } else if (node instanceof BinaryExpression) {
      this.endVisitBinaryExpression(node);
    } else if (node instanceof CastExpression) {
      this.endVisitCastExpression(node);
    } else if (node instanceof CommaExpression) {
      this.endVisitCommaExpression(node);
    } else if (node instanceof FunctionCallExpression) {
      this.endVisitFunctionCallExpression(node);
    } else if (node instanceof PostfixExpression) {
      this.endVisitPostfixExpression(node);
    } else if (node instanceof PrefixExpression) {
      this.endVisitPrefixExpression(node);
    } else if (node instanceof PropertyAccessExpression) {
      this.endVisitPropertyAccessExpression(node);
    } else if (node instanceof SizeofExpression) {
      this.endVisitSizeofExpression(node);
    } else if (node instanceof TernaryExpression) {
      this.endVisitTernaryExpression(node);
    } else if (node instanceof UnaryExpression) {
      this.endVisitUnaryExpression(node);
    } else if (node instanceof ValueExpression) {
      this.endVisitValueExpression(node);
    } else {
      throw new Error();
    }
  }

  public visitValueExpression(node: ValueExpression): boolean {
    if (node instanceof ArrayValueExpression) {
      return this.visitArrayValueExpression(node);
    }
    if (node instanceof BooleanValueExpression) {
      return this.visitBooleanValueExpression(node);
    }
    if (node instanceof IdentifierValueExpression) {
      return this.visitIdentifierValueExpression(node);
    }
    if (node instanceof NumberValueExpression) {
      return this.visitNumberValueExpression(node);
    }
    if (node instanceof StringValueExpression) {
      return this.visitStringValueExpression(node);
    }
    throw new Error();
  }

  public endVisitValueExpression(node: ValueExpression): void {
    if (node instanceof ArrayValueExpression) {
      this.endVisitArrayValueExpression(node);
    } else if (node instanceof BooleanValueExpression) {
      this.endVisitBooleanValueExpression(node);
    } else if (node instanceof IdentifierValueExpression) {
      this.endVisitIdentifierValueExpression(node);
    } else if (node instanceof NumberValueExpression) {
      this.endVisitNumberValueExpression(node);
    } else if (node instanceof StringValueExpression) {
      this.endVisitStringValueExpression(node);
    }
    throw new Error();
  }
}
